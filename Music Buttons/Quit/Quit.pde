...
void keyPressed() {
  ...
  if ( key=='k'  || key=='k' ) ; song[currentSong].loop(-1);//loop infintity//parameter:blank or -1
  if ( key=='R'  || key=='f' )song[currentSong].skip(10000) ; // fast forward//parameter:
  if ( key== 'F'  || key=='r' ) ; //fast reverse
  if ( key=='m' || key=='m' ) ; //mute
